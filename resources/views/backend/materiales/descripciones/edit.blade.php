  ...
  <div class="row">
    <div class="col-md">
      ...
      <div class="form-group">
        ...
        {!! Form::select('materiale_id', \App\Materiale::pluck('nombre','id'), null, ['class' => 'form-control col-md-2','placeholder'=>'Material', 'v-model' => 'materiale_id']) !!}
      ...
      <div class="form-group">
        ...
        {!! Form::text('descripcion', null, ['class' => 'form-control col-md-4', 'placeholder' => 'Descripcion', 'v-model'=> 'descripcion']) !!}
      ...
      <div class="form-row">
        <div class="form-group mr-2">
          {!! Form::text('flargo', null, ['class' => 'form-control','placeholder'=>'Formula de Largo', 'v-model'=> 'flargo']) !!}
          {!! $errors->first('flargo', '<small class="help-block text-danger">:message</small>') !!}
        </div>
        <div class="form-group mr-2">
          {!! Form::text('fancho', null, ['class' => 'form-control','placeholder'=>'Formula de Ancho', 'v-model'=> 'fancho']) !!}
          {!! $errors->first('fancho', '<small class="help-block text-danger">:message</small>') !!}
        </div>
        <div class="form-group mr-2">
          {!! Form::text('espesor', null, ['class' => 'form-control','placeholder'=>'Formula de Espesor', 'v-model'=> 'espesor']) !!}
...
<script>
  var app = new Vue({
    ...
    created() {
      axios.get('/Tipos').then( response => { this.tiposList = response.data }).catch( function(error) { console.log(error) });
      axios.get('/editData/' + '{{ $descripcione->id }}')
      .then( response => {
        this.materiale_id = response.data.materiale_id;
        this.descripcion = response.data.descripcion;
        this.flargo = response.data.flargo;
        this.fancho = response.data.fancho;
        this.espesor = response.data.espesor;
